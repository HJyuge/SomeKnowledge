...
- (void)putInTransactionArray:(NSArray *)array intoTable:(NSString *)tableName objectIDKey:(NSString *)objectId
{
	if ([YTKKeyValueStore checkTableName:tableName] == NO) {
		return;
	}
	...
	[self.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
		for (id object in array) {
			...
			NSDate * createdTime = [NSDate date];
			NSString * sql = [NSString stringWithFormat:UPDATE_ITEM_SQL, tableName];
			NSString * key = [object objectForKey:objectId];
			res =   [db executeUpdate:sql, key, jsonString, createdTime] ;
			if (!res) {
				*rollback = YES;
			}
...
- (void)deleteInTransactionArray:(NSArray *)array intoTable:(NSString *)tableName objectIDKey:(NSString *)objectId
{
	if ([YTKKeyValueStore checkTableName:tableName] == NO) {
		return;
	}
